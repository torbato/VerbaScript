...
def run_whisper_transcription_process(temp_audio_path_str, output_folder_str, video_stem, model_name, lang_whisper, device_choice, use_fp16, result_queue):
    ...
    try:
        ...
        # This print will go to the console from where the main app was launched,
        # or be lost if running as a bundled .exe without console.
        ...
        transcribe_options = {"fp16": use_fp16, "verbose": False} # Keep verbose False for library use
        ...
        result_queue.put(True) # Indicate success
        return True
    except Exception as e:
        print(f"[Whisper Process] Error during transcription for {video_stem}: {e}")
        import traceback
        traceback.print_exc() # Print full traceback from child process
        result_queue.put(False) # Indicate failure
        return False
...
class TranscriberApp:
    def __init__(self, root_tk):
        ...
        self.is_processing = False # Shared flag to signal stop
        self.processing_thread = None
        self.ffmpeg_process = None 
        self.whisper_mp_process = None # For the multiprocessing Process object
    ...
    def setup_ui(self):
        ...
        self.log_frame.pack(expand=True, fill=tk.BOTH, pady=5)
    ...
    def set_ui_state(self, processing):
        self.is_processing = processing # Set the flag first
            ...
            for child in self.main_frame.winfo_children():
                if isinstance(child, (ttk.LabelFrame, ttk.Frame)):
                    for widget in child.winfo_children():
                     ...
                     if child not in [self.start_button, self.stop_button, self.progress_bar]:
    ...
    def start_processing(self):
        ...
        self.set_ui_state(True) # Set is_processing to True
    ...
    def stop_processing(self):
        ...
        self.is_processing = False # Primary flag to stop loops
        ...
        if self.whisper_mp_process and self.whisper_mp_process.is_alive():
            ...
            try: self.whisper_mp_process.terminate(); self.whisper_mp_process.join(timeout=1) # Wait a bit for termination
            except Exception as e: self.log_message(f"Error terminating Whisper process: {e}", "error")
        self.whisper_mp_process = None
        
        self.stop_button.config(state=tk.DISABLED)
        # The processing thread will check self.is_processing and should exit its loops.
        # UI state (like enabling start button) will be handled by the thread's finally block.
    ...
    def _process_videos_thread(self, video_files, output_folder_str, model_name, lang_whisper, device_choice):
        ...
        # Model is loaded in the child process, not here.
        ...
        try:
            ...
            for i, video_path in enumerate(video_files):
                ...
                if num_total_files > 0: self.root.after(0, lambda p=files_processed_count: self.progress_var.set((p / num_total_files) * 100))
                ...
                try:
                    ...
                    # Create a queue for the child process to send back result status
                    ...
                    # Wait for the Whisper process to finish, but check self.is_processing
                    while self.whisper_mp_process.is_alive():
                        if not self.is_processing:
                            ...
                            self.whisper_mp_process.join(timeout=2) # Wait for termination
                            break 
                        time.sleep(0.2) # Check every 200ms
                    
                    if not self.is_processing: # If loop was broken by stop signal
                        self.log_message(f"Whisper process for {video_path.name} handled after stop request.", "debug")
                        # Result from queue might not be reliable if terminated early
                        try: result_queue_mp.get_nowait() # Try to clear queue if anything was put
                        except queue.Empty: pass
                        break # Break from the main files loop

                    # If process finished normally (not terminated by stop signal)
                    transcription_succeeded = False
                    try:
                        transcription_succeeded = result_queue_mp.get(timeout=5) # Wait for result from queue
                    except queue.Empty:
                        self.log_message(f"Whisper process for {video_path.name} did not return a result in time.", "warning")
                    
                    self.whisper_mp_process = None # Clear process reference
                    ...
                    # Ensure subprocess references are cleared if an exception occurred before they were naturally cleared
                    ...
                    if num_total_files > 0 and self.is_processing : 
                        self.root.after(0, lambda p=files_processed_count: self.progress_var.set((p / num_total_files) * 100))
            ...
            self.is_processing = False # Ensure flag is definitively False
            self.root.after(0, lambda: self.set_ui_state(False)) # Update UI state on main thread
            if files_processed_count > 0 : self.root.after(0, lambda: self.progress_var.set(100)) 
...
def main():
    # It's crucial for multiprocessing with 'spawn' (often default/safer on Windows/macOS)
    # that set_start_method is called only once and ideally within `if __name__ == '__main__':`
    # before any multiprocessing objects are created.
    if sys.platform in ['win32', 'darwin']: # 'spawn' is default on Windows, good for macOS too
